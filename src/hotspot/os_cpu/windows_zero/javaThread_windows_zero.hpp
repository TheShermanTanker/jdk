...
#define OS_CPU_WINDOWS_ZERO_JAVATHREAD_WINDOWS_ZERO_HPP
  ...
  ZeroStack* zero_stack() throw() {
    return &stack;
  }

  ZeroFrame* top_zero_frame() const throw() {
    return top;
  }

  void push_zero_frame(ZeroFrame* frame) throw() {
  ...
  bool has_special_condition_for_native_trans() const throw() {
