...
[
  ...
  if test "x$TOOLCHAIN_TYPE" = xgcc && test "x$OPENJDK_TARGET_OS" = xwindows; then
    ...
    UTIL_GET_NON_MATCHING_VALUES(TOOLCHAIN_CFLAGS_JVM, $TOOLCHAIN_CFLAGS_JVM, -fvisibility=hidden -fstack-protector)
    UTIL_GET_NON_MATCHING_VALUES(TOOLCHAIN_CFLAGS_JDK, $TOOLCHAIN_CFLAGS_JDK, -fstack-protector)
    TOOLCHAIN_CFLAGS_JVM="$TOOLCHAIN_CFLAGS_JVM -fno-stack-protector"
    TOOLCHAIN_CFLAGS_JDK="$TOOLCHAIN_CFLAGS_JDK -fno-stack-protector"
  fi
